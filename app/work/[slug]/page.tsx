...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="case-study">
      ...
      {incidents && <section className="incidents-block reveal"><div className="shell case-section"><p className="eyebrow">INCIDENTS THAT CHANGED THE ARCHITECTURE</p><div className="incident-grid">{incidents.cases.map((incident, i) => <article key={incident.title}><span>0{i + 1}</span><h2>{incident.title}</h2><dl><div><dt>SIGNAL</dt><dd>{incident.signal}</dd></div><div><dt>DIAGNOSIS</dt><dd>{incident.diagnosis}</dd></div><div><dt>RESPONSE</dt><dd>{incident.response}</dd></div><div><dt>PREVENTION</dt><dd>{incident.prevention}</dd></div></dl></article>)}</div><aside className="additional-hardening"><span>ADDITIONAL HARDENING</span><p>{incidents.additionalHardening}</p></aside></div></section>}

      {project.reliability && <section className="reliability-block reveal"><div className="shell case-section"><p className="eyebrow">PRODUCTION RELIABILITY</p><h2>{project.reliability.title}</h2><p className="reliability-copy">{project.reliability.body}</p><div className="evidence-grid">{project.reliability.evidence.map((item) => <article key={item.value}><strong>{item.value}</strong><p>{item.label}</p></article>)}</div></div></section>}
